-- Fix matchmaking room-code generation in environments where extensions schema is not on search_path
-- Root cause: matchmake_two_player sets search_path to public only, so gen_random_bytes isn't resolvable.
...
BEGIN
  ...
  LOOP
    ...
    v_room_code := upper(substr(encode(extensions.gen_random_bytes(2), 'hex'), 1, 4));
